...
//   "balanced"  — production default (sonnet-tier)
//   "fast"      — cheap/quick (haiku-tier)
// Legacy claude-* names remain supported (passed through unchanged).
const TIER_TO_ANTHROPIC = {
  flagship: process.env.ANTHROPIC_MODEL_FLAGSHIP || 'claude-opus-4-7',
...
function normalizeTier(raw) {
  if (!raw) return null;  // caller didn't specify; leave unchanged
  ...
  return null;  // not a tier name → preserve caller's literal
}

// Rewrite the request body's `model` field if it's a tier name.
// Legacy claude-* names pass through untouched so Anthropic's API
// resolves them directly (supports date-suffixed variants like
// claude-haiku-4-5-20251001 that only Anthropic understands).
function resolveModelInBody(bodyStr) {
  let obj;
  try { obj = JSON.parse(bodyStr); } catch (_) { return bodyStr; }
  const tier = normalizeTier(obj && obj.model);
  if (!tier) return bodyStr;
  obj.model = TIER_TO_ANTHROPIC[tier];
...
function startServer(config) {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      // Tier resolution — if the caller sent "flagship"/"balanced"/"fast"
      // in the `model` field, rewrite to the actual Anthropic model name
      // before any further processing. Legacy claude-* names pass through.
      bodyStr = resolveModelInBody(bodyStr);
